...
neg.llhd.general <- function(s, DCT3y)
{
    if (min(s) < 0)
        ...
        n <- dim(DCT3y)
        Lambda <- ml.setup.eigvals(s, n)
        Gamma <- 1 + ifelse(Lambda == 0, 0, 1/Lambda)
        sum(DCT3y^2/Gamma + log(Gamma))
    }
}

ml.smooth3d.general <- function(y, initval)
{
    if (length(dim(y)) != length(initval))
        ...
        lambda <- ml.setup.eigvals(shat, n)
        gamma <- 1/(1 + lambda)
